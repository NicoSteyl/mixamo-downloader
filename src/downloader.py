...
class MixamoDownloader(QtCore.QObject):
    ...
    def __init__(self, path, mode, rewrite, query=None):
        ...
        :param rewrite: Overwrite existing files
        :type rewrite: bool
    ...
    def run(self):
        ...
        if self.mode == "tpose":
            ...
            tpose_payload = self.build_tpose_payload(
    ...
    def download_animation(self, url):
        ...
        if url:
            ...
            forbidden_chars = ["<", ">", ":", "\"", "/", "\\", "|", "?", "*"]
            fixed_name = "".join(
                char for char in self.product_name if char not in forbidden_chars)

            filename = f"{self.path}/{fixed_name}.fbx"
            ...
            # (i.e: the folder where this Python script is being executed).
                ...
                open(f"{fixed_name}.fbx", "wb").write(response.content)
